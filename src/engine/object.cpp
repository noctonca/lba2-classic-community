...
T_COMPORTEMENT ListComportement[MAX_COMPORTEMENTS] = {
    ...
    {1, -1, NULL, CF_COMPORTEMENT // Protopack
                      | CF_INVENTORY | CF_WALK_ON_CODES_JEU | CF_WEAPON | CF_HOLOMAP | CF_SHIELD},
    {2, -1, NULL, CF_HOLOMAP},    // Double
    {3, -1, NULL, CF_COMPORTEMENT // Conque
                      | CF_INVENTORY | CF_WEAPON | CF_HOLOMAP | CF_SHIELD},
    {4, -1, NULL, CF_COMPORTEMENT // ScaphIntNorm
                      | CF_WEAPON | CF_HOLOMAP},
    {1, -1, NULL, CF_COMPORTEMENT | CF_INVENTORY | CF_WALK_ON_CODES_JEU | CF_WEAPON | CF_HOLOMAP | CF_SHIELD},
...
*/
    if (ptrobj->Move == MOVE_CIRCLE OR ptrobj->Move == MOVE_CIRCLE2) {
        S32 x, z;
    ...
    if (ptrobj->Move == MOVE_WAGON OR ptrobj->Move == MOVE_BUGGY_MANUAL) {
...
void SetComportement(U8 comportement) {
    ...
    if (comportement >= MAX_COMPORTEMENTS)
        return;
...
void RestartPerso() {
    ...
    if (MemoMove == MOVE_BUGGY_MANUAL OR MemoMove == MOVE_BUGGY) {
...
void ClearFoudre(void) {
    ...
    for (n = 0; n < MAX_INCRUST_DISP; n++, ptrdisp++) {
        ...
        if (ptrdisp->Num != -1 AND(ptrdisp->Type & 0xFF) == INCRUST_ECLAIR) {
            ptrdisp->Num = -1;
            Palette(PtrPal);
        }
    }
...
#define CleanHQR(hqr)                                \
    if (hqr && (hqr->FreeSize < (hqr->MaxSize / 3))) \
    HQR_Reset_Ressource(hqr)
...
void InitLoadedGame() {
    ...
    if ((ptrobj->CarryBy != -1)
        ...
        ptrobj->PtrZoneRail = (T_ZONE *)(uintptr_t)(U32)Distance2D(ListObjet[nu].Obj.X, ListObjet[nu].Obj.Z,
...
void ChangeCube() {
    ...
    if (FlagReinit) {
        ...
        if (MemoFrame < ptrobj->Obj.NbFrames - 2)
            MemoFrame++;
    ...
    if (FlagReajustPosTwinsen
        ...
        if (CubeMode == CUBE_EXTERIEUR) {
            ...
            GetShadow(SceneStartX, SceneStartY, SceneStartZ);
    ...
    if (Island > 3)
        TabInv[FLAG_MONEY].IdObj3D = 1;
    else
        TabInv[FLAG_MONEY].IdObj3D = 0;
    ...
    if (FlagLoadGame) {
        ...
        /* Issue #62 harness: surface flagload before any post-load step that might
         * crash, so the driver script always sees the load outcome. */
        if (SaveLoadTestMode) {
            fprintf(stdout, "SAVE_LOAD_TEST: stage=after_loadgame flagload=%d numcube=%d\n",
                    (int)flagload, (int)NumCube);
            fflush(stdout);
        }

        if (flagload == LOADGAME_ERR_CONTEXT) {
            /* LoadContexte failed mid-stream; ListObjet may be partial — InitLoadedGame would SIGSEGV. */
        } else if (flagload < 0 OR !flagload)
            InitLoadedGame();
        else {
            LoadFile3dObjects();
        }
...
void InitHitExtraPof(S32 x, S32 y, S32 z, S32 hitforce) {
    ...
    while (hitforce >= 1 AND nb < MAX_HIT_POFS) // on limite à 5 étoiles
    {
        ...
        if ((n != numobj)
            ...
            if (x0<xt1
                ...
                if (ptrobj->Move == MOVE_WAGON
                                        AND !(ListObjet[n].Flags & SPRITE_3D))
                    return n;
                ...
                if (ptrobjt->Flags & OBJ_CARRIER) {
                    if ((y0 <= (yt1 + 1)) // CheckZvOnZv( numobj, n )
                        ...
                        if (ptrobj->CarryBy != n AND ptrobj->Move != MOVE_WAGON) // Ne peut pas être porté !!!!
                        // ne devrait jamais arriver ici
                        {
                            ...
                            ptrobj->PtrZoneRail = (T_ZONE *)(uintptr_t)(U32)Distance2D(ptrobjt->Obj.X, ptrobjt->Obj.Z, Nxw, Nzw);
                            ptrobj->CarryBy = n;
                        }
                        continue;
                    }
                } else {                  // je marche sur quelqu'un
                    if ((y0 <= (yt1 + 1)) // CheckZvOnZv( numobj, n )
                        AND(y0 > (yt1 - SIZE_BRICK_Y))
                    ...
                    AND(!IsBuggyPresent() OR n != NUM_BUGGY OR numobj != NUM_PERSO)) // Twinsen ne peut pas pousser le buggy
                {
                    ...
                    if (ptrobjt->Flags & MINI_ZV) { // magouille boxxle align pos
                        ...
                        if (angle >= (896 * MUL_ANGLE) OR angle < (128 * MUL_ANGLE))
                            if (ptrobj->Obj.Beta >= (896 * MUL_ANGLE) OR ptrobj->Obj.Beta < (128 * MUL_ANGLE))
                                ptrobjt->Coord.Push.PushZ = SIZE_BRICK_XZ / 8;
                    } else { // induit deplacement avant reajustement ?
...
*/
                ...
                if (ptrobj->Flags & OBJ_CARRIER AND flagy) {
                    ...
                    dx = ade_min(abs(xt1 - x0), abs(x1 - xt0));
                    dz = ade_min(abs(zt1 - z0), abs(z1 - zt0));
                ...
                test_next_coin:
                    ...
                    if (flagy) // je suis rentré par dessus
                    {
                        if (PtrWorldColBrick(Nxw, Nyw, Nzw) != 0) {
                            ...
                            goto test_obj_zv; // pos acceptée, on reteste cet objet
                    }
                }
            }
        }
        ...
        if (numobj == NUM_PERSO AND Pushing AND Pushing == n) {
            ...
            if (abs(ptrobj->Obj.Beta - angle) > 2048)
                angle -= MAX_ANGLE;
...
S32 CheckValidObjPos(U8 numobj, U8 owner) {
    ...
    if ((ptrobj->Obj.X < DEMI_BRICK_XZ) OR(ptrobj->Obj.X > SIZE_BRICK_XZ * 64 - DEMI_BRICK_XZ - 1))
        return FALSE;
    //	if( (ptrobj->Obj.Y<DEMI_BRICK_Y)  OR (ptrobj->Obj.Y>SIZE_BRICK_Y*64-DEMI_BRICK_Y-1) )	return FALSE ;
    if ((ptrobj->Obj.Z < DEMI_BRICK_XZ) OR(ptrobj->Obj.Z > SIZE_BRICK_XZ * 64 - DEMI_BRICK_XZ - 1))
        return FALSE;
    ...
    if (CubeMode == CUBE_INTERIEUR) {
        // check point chaud
        if (WorldColBrickFull(ptrobj->Obj.X, ptrobj->Obj.Y, ptrobj->Obj.Z, ptrobj->YMax))
            return FALSE;
        ...
        // check point chaud
        if (WorldColBrickExt(ptrobj->Obj.X, ptrobj->Obj.Y, ptrobj->Obj.Z) == 1)
            return FALSE;

        // check zv
        if (WorldColBrickExt(x0, y0, z0) == 1)
            return FALSE;
        if (WorldColBrickExt(x1, y0, z0) == 1)
            return FALSE;
        if (WorldColBrickExt(x1, y0, z1) == 1)
            return FALSE;
        if (WorldColBrickExt(x0, y0, z1) == 1)
            return FALSE;

        // teste ZV dans ZV des objets du decors
        if (WorldColBrickDecors(x0, y0, z0, x1, y1, z1) == 1)
            return FALSE;
...
void PosObjetAroundAnother(U8 numsrc, U8 numtopos) {
    ...
    dist = ade_max(objsrc->XMin, objsrc->XMax);
    dist = ade_max(dist, objsrc->ZMin);
    dist = ade_max(dist, objsrc->ZMax);

    dist2 = ade_max(objtopos->XMin, objtopos->XMax);
    dist2 = ade_max(dist2, objtopos->ZMin);
    dist2 = ade_max(dist2, objtopos->ZMax);
...
void InitBody(U8 gennewbody,
    ...
    if (numobj == NUM_PERSO)
        gennewbody = ChoiceHeroBody(gennewbody);
    ...
    if (newbody != -1) {
        if (newbody != ptrobj->Obj.Body.Num) {
            ...
            ObjectInitBody(&ptrobj->Obj, (void *)(uintptr_t)(U32)newbody);
...
void InitSprite(S32 newsprite,
    ...
    if (newsprite != -1 AND newsprite != ptrobj->Obj.Body.Num) {
        ...
        if (ptrobj->Flags & ANIM_3DS)
            ptr = &PtrZvAnim3DS[newsprite * 8 + 2];
        else {
            if (newsprite >= 100)
                ptr = &PtrZvExtra[newsprite * 8 + 2];
            else
                ptr = &PtrZvExtraRaw[newsprite * 8 + 2];
        }
...
S32 InitAnim(U16 gennewanim,
    ...
    ObjectInitAnim(&ptrobj->Obj, (void *)(uintptr_t)(U32)newanim);

    ptrobj->GenAnim = gennewanim;

    ptrobj->PtrAnimAction = SearchPtrAnimAction;

    ptrobj->WorkFlags &= ~(OK_HIT | OK_SUPER_HIT | ANIM_END | FRAME_SHIELD | OK_RENVOIE | LEFT_JUMP | RIGHT_JUMP | WAIT_SUPER_HIT);
...
void ReajustPos(U8 col) {
    ...
    if (col <= 1)
        return; //  - optimisation puisque on ne
...
void ReceptionObj() {
    if (AnimNumObj == NUM_PERSO) {
        ...
        if (StartYFalling - Nyw >= 16 * SIZE_BRICK_Y) { // trop haut mort directe
        ...
        } else if (StartYFalling - Nyw >= 8 * SIZE_BRICK_Y) { // se fait mal
            InitHitExtraPof(APtObj->Obj.X,
                            APtObj->Obj.Y + APtObj->YMax + 1,
                            APtObj->Obj.Z,
                            5);
            ...
            APtObj->LifePoint -= 5;
            InitAnim(GEN_ANIM_RECEPTION_2, ANIM_ALL_THEN, NUM_PERSO);
        } else if (StartYFalling - Nyw > 1) { // reception normale
...
S32 CheckCarryBy(U8 numobj) {
    ...
    if (nu != -1) {
        if (!(nu & CARRY_BY_DECORS)) {
            ...
            if (Nxw != APtObj->Obj.X OR Nzw != APtObj->Obj.Z) {
                ...
                // car on se sert du champ PtrZoneRail
                {
                    ...
                    APtObj->PtrZoneRail = (T_ZONE *)(uintptr_t)(U32)Distance2D(ptrobjc->Obj.X, ptrobjc->Obj.Z, Nxw, Nzw);
                }
            }
                                                      ...
                                                      AND abs(ptrobjc->Obj.Beta - ptrobjc->OldBeta) <= (256 * MUL_ANGLE)) // l'objet porteur tourne
            {
                ...
                LongRotate(0, (U64)APtObj->PtrZoneRail, dangle);
                ...
                if (APtObj->Move != MOVE_MANUAL AND APtObj->Move != MOVE_WAGON) {
    ...
    if ((APtObj->Flags & CHECK_CODE_JEU)
        ...
        if (APtObj->WorkFlags & DONT_PICK_CODE_JEU) {
            ...
            if ((APtObj->Flags & SPRITE_3D)
                ...
                if ((PtrWorldCodeBrick(Nxw + APtObj->XMin, Nyw - 1, Nzw + APtObj->ZMin) >> 4) != oldcodejeu AND(PtrWorldCodeBrick(Nxw + APtObj->XMin, Nyw - 1, Nzw + APtObj->ZMax) >> 4) != oldcodejeu AND(PtrWorldCodeBrick(Nxw + APtObj->XMax, Nyw - 1, Nzw + APtObj->ZMin) >> 4) != oldcodejeu AND(PtrWorldCodeBrick(Nxw + APtObj->XMax, Nyw - 1, Nzw + APtObj->ZMax) >> 4) != oldcodejeu)
                ...
                if (!FlagWater) {
                    ...
                    default:
                        if (ModeLabyrinthe) {
                            ...
                            InitAnim(GEN_ANIM_NOYADE, ANIM_FINAL, NUM_PERSO);
                    }
        ...
        case CJ_ELECTRIC: // mort (rails en montee par ex.)
            ...
            if (numobj == NUM_PERSO) {
                ...
                if (APtObj->GenAnim == GEN_ANIM_CHOC OR APtObj->GenAnim == GEN_ANIM_CHOC2 OR APtObj->GenAnim == GEN_ANIM_SAUTE OR APtObj->GenAnim == GEN_ANIM_SAUTE_DROIT OR APtObj->GenAnim == GEN_ANIM_SAUTE_GAUCHE)
                    break;
        ...
        case CJ_POS_INVALID: // Position non valide pour la mort
            ...
            if (numobj == NUM_PERSO AND APtObj->CarryBy == -1) {
                validepos = FALSE;
            }
            break;
        }
    ...
    if (APtObj->Obj.Anim.Num != -1) {
        ...
        if (flag & FLAG_FRAME) // Nouvelle frame
        {
            ...
            if (flag & FLAG_LAST_FRAME) {
                APtObj->WorkFlags &= ~(OK_HIT | OK_SUPER_HIT | OK_RENVOIE | LEFT_JUMP | RIGHT_JUMP | WAIT_SUPER_HIT);
                ...
                if (APtObj->FlagAnim != ANIM_REPEAT) {
                    ...
                    ObjectInitAnim(&APtObj->Obj, (void *)(uintptr_t)(U32)numanim);
            ...
            if (APtObj->WorkFlags & AUTO_STOP_DOOR) {
                ...
                if (APtObj->DoorWidth) {
                    ...
                    if (Distance2D(Nxw, Nzw,
                                   ...
                                   APtObj->Obj.LastAnimStepZ) >= APtObj->DoorWidth) {
                        // recalage violent
                ...
                // DoorWidth
                    ...
                    switch (APtObj->Obj.Beta) {
                    case (768 * MUL_ANGLE): // left
                        if (Nxw >= APtObj->Obj.LastAnimStepX)
                            flag = TRUE;
                        break;
                    case (256 * MUL_ANGLE): // right
                        if (Nxw <= APtObj->Obj.LastAnimStepX)
                        ...
                        break;
                    case (512 * MUL_ANGLE): // up
                        if (Nzw >= APtObj->Obj.LastAnimStepZ)
                            flag = TRUE;
                        break;
                    case (0 * MUL_ANGLE): // down
                        if (Nzw <= APtObj->Obj.LastAnimStepZ)
    ...
    if ((APtObj->Flags & ANIM_3DS)
        ...
        if (APtObj->Coord.A3DS.Fin >= APtObj->Coord.A3DS.Deb) {
            ...
            newsprite = BoundRegleTrois(
                ...
                TimerRefHR - (U64)APtObj->PtrFile3D);

            if (newsprite > APtObj->Coord.A3DS.Fin) {
                newsprite = APtObj->Coord.A3DS.Deb;
                APtObj->PtrFile3D = (U8 *)(uintptr_t)(U32)TimerRefHR;
            }
        } else {
...
*/
        ...
        if ((AnimNumObj == NUM_PERSO) AND((ptrobj->Flags & COL_BASSE) == 0)) {
            if (!DoCornerReajustTwinsen(APtObj->XMin, APtObj->YMin, APtObj->ZMin))
                validepos = FALSE;
            if (!DoCornerReajustTwinsen(APtObj->XMax, APtObj->YMin, APtObj->ZMin))
                validepos = FALSE;
            if (!DoCornerReajustTwinsen(APtObj->XMax, APtObj->YMin, APtObj->ZMax))
                validepos = FALSE;
            if (!DoCornerReajustTwinsen(APtObj->XMin, APtObj->YMin, APtObj->ZMax))
                validepos = FALSE;
                    ...
                    if (!(APtObj->WorkFlags & ANIM_MASTER_GRAVITY)) {
                        ...
                        if (ptrobj->ObjCol == 255) {
                            if (APtObj->Move == MOVE_WAGON) {
                                ...
                                if (APtObj->Flags & SPRITE_3D) {
                                    if (WorldColBrick(Nxw + APtObj->XMin, Nyw - 1, Nzw + APtObj->ZMin) != 1 AND WorldColBrick(Nxw + APtObj->XMin, Nyw - 1, Nzw + APtObj->ZMax) != 1 AND WorldColBrick(Nxw + APtObj->XMax, Nyw - 1, Nzw + APtObj->ZMin) != 1 AND WorldColBrick(Nxw + APtObj->XMax, Nyw - 1, Nzw + APtObj->ZMax) != 1) {
                                        APtObj->WorkFlags |= FALLING;
                                    }
                                } else // obj 3D
                                {
    ...
    if (Nxw < DEMI_BRICK_XZ)
        Nxw = DEMI_BRICK_XZ;
    if (Nzw < DEMI_BRICK_XZ)
        Nzw = DEMI_BRICK_XZ;

    // mettre a jour test a 0

    // Grosse Magouille Cage de la fin
    if (Nyw < 0 AND !(APtObj->Flags & OBJ_ZBUFFER))
        Nyw = 0;

    // a mettre en commentaire ????
    // peut-etre pour les objets sortants du cube
    if (Nxw > 64 * SIZE_BRICK_XZ - DEMI_BRICK_XZ)
        Nxw = 64 * SIZE_BRICK_XZ - DEMI_BRICK_XZ;
    if (Nzw > 64 * SIZE_BRICK_XZ - DEMI_BRICK_XZ)
        Nzw = 64 * SIZE_BRICK_XZ - DEMI_BRICK_XZ;
...
findoanim:
    ...
    if (numobj == NUM_PERSO) {
        ...
        // pour le protopack
        if (Comportement == C_PROTOPACK AND APtObj->GenAnim == GEN_ANIM_MARCHE) {
            validepos = FALSE;
        }
...
void ManualRealAngle(T_OBJET *ptrobj) {
    if (Input & I_LEFT)
        ChangeSpeedMove(&ptrobj->BoundAngle.Move, 1024);
    else if (Input & I_RIGHT)
        ChangeSpeedMove(&ptrobj->BoundAngle.Move, -1024);
    else
        ChangeSpeedMove(&ptrobj->BoundAngle.Move, 0);
...
void DoDir(U8 numobj) {
    ...
    if (!(ptrobj->Flags & SPRITE_3D) AND ptrobj->Move != MOVE_WAGON AND ptrobj->Move != MOVE_MANUAL AND ptrobj->Move != MOVE_BUGGY_MANUAL) {
    ...
    case MOVE_MANUAL:
        ...
        if (numobj == NUM_PERSO) /* protection anim speciales */
        {
            ...
            case C_SCAPH_EXT_SPOR: // Scaphandre Sportif Exterieur
                if (!Jumping
                         AND ptrobj->GenAnim == GEN_ANIM_MARCHE AND ptrobj->Obj.LastFrame == 1 - 1) {
                    Jumping = 1024;
                }
            ...
            case C_PROTOPACK:
                ...
                if (/*ptrobj->GenAnim!=GEN_ANIM_MARCHE
			AND*/
                    (ptrobj->CodeJeu >> 4) == CJ_FOOT_WATER
                ...
                if ((Input & I_THROW)
                    ...
                    case FLAG_SARBACANE:
                        ...
                        if (TabInv[FLAG_SARBACANE].IdObj3D == 0) // non
                            ...
                            InitAnim(GEN_ANIM_SARBATRON, ANIM_REPEAT, numobj);
                        flagattack = TRUE;
        ...
        if (Jumping
                AND(ptrobj->GenAnim == GEN_ANIM_RIEN OR ptrobj->GenAnim == GEN_ANIM_RECEPTION OR ptrobj->WorkFlags & ANIM_MASTER_ROT)) {
            Jumping = FALSE;
        }
        ...
        if (!(Input & I_FIRE) OR ActionNormal) {
            ...
            if (Input & (I_UP | I_DOWN)) {
                if (Jumping & 1023 AND LastMyJoy & (I_UP | I_DOWN)
                    //			AND !PtrZoneClimb )
                    AND !FlagClimbing) {
            ...
            if (test_down AND(Input & I_DOWN)) {
                if (FlagClimbing != CLIMBING_DOWN AND ptrobj->GenAnim != GEN_ANIM_ECHDESC) // Descente d'échelle ?
                {
                    if (Input & I_ESQUIVE) {
                        if (ptrobj->FlagAnim != ANIM_ALL_THEN) {
                        ...
                        InitAnim(GEN_ANIM_RECULE, ANIM_REPEAT, numobj);
            ...
            if ((Input & I_LEFT) AND !(Jumping & 1023)) {
                ...
                if (ptrobj->GenAnim == GEN_ANIM_RIEN) {
                    if (Input & I_ESQUIVE) {
                        ...
                        InitAnim(GEN_ANIM_GAUCHE, ANIM_REPEAT, numobj);
                } else {
                    if (!(ptrobj->WorkFlags & ANIM_MASTER_ROT)) {
                        ptrobj->Obj.Beta += GetDeltaMove(&ptrobj->BoundAngle.Move);
                        ptrobj->Obj.Beta &= 4095;
                ...
                if (ptrobj->GenAnim == GEN_ANIM_RIEN) {
                    if (Input & I_ESQUIVE) {
                        ...
                        InitAnim(GEN_ANIM_DROITE, ANIM_REPEAT, numobj);
                } else {
                    if (!(ptrobj->WorkFlags & ANIM_MASTER_ROT)) {
                        ptrobj->Obj.Beta += GetDeltaMove(&ptrobj->BoundAngle.Move);
                        ptrobj->Obj.Beta &= 4095;
...
*/
            ...
            DrawRecover(pt->Xw, yw, pt->Zw,
                        pt->YMax - yw,
                        ade_max((xmax - xmin) / 2, (zmax - zmin) / 2));
        }
    }
}
...
*/
                ...
                if (ptrobj->Flags & (OBJ_ZBUFFER | OBJ_IN_WATER)) {
                    ...
                    if (DrawRecover(ptrobj->Obj.X, yw, ptrobj->Obj.Z, ptrobj->YMax,
                                    ade_max((ptrobj->XMax - ptrobj->XMin) / 2, (ptrobj->ZMax - ptrobj->ZMin) / 2))) {
        ...
        if (AffParticleFlow(ptrf)) {
            ...
            if (ClipXMin <= ClipXMax
                ...
                DrawRecover(xw, yw, zw, ptrf->YMax,
                            ade_max((ptrf->XMax - ptrf->XMin) / 2, (ptrf->ZMax - ptrf->ZMin) / 2));
            }
        }
    } break;
        ...
        if (aff) {
            ...
            if (ClipXMin <= ClipXMax
                ...
                DrawRecover(ptrd->PosX + ptrd->XMax, yw, ptrd->PosZ + ptrd->ZMax, ptrd->YMax,
                            ade_max((ptrd->XMax - ptrd->XMin) / 2, (ptrd->ZMax - ptrd->ZMin) / 2));
            }
        }
        break;
...
startaffscene:
...
#ifdef ENABLE_POLY_RECORDING
    // Polygon call recording (Alt+F9) — start recording before ANY rendering
    // (terrain + objects). Must be before RefreshGrille/DrawAnimatedPolys.
    static char polyrec_pending_name[256] = {0};
    if (SnapshotRequested) {
        static int polyrec_counter = 0;
        snprintf(polyrec_pending_name, sizeof(polyrec_pending_name), "polyrec_%04d.lba2polyrec", polyrec_counter++);
        polyrec_start(polyrec_pending_name);
        // Provide RGB palette for PPM output
        {
            U8 *active_pal = PtrPalCurrent ? PtrPalCurrent : PalettePcx;
            if (active_pal)
                polyrec_set_rgb_palette(active_pal);
        }
        SnapshotRequested = 0;
    } else {
        polyrec_pending_name[0] = 0;
    }
#endif
...
#ifdef ENABLE_POLY_RECORDING
    // When recording, force a full redraw (terrain + objects) so the recording
    // captures all polygon calls. Without this, AFF_OBJETS_* modes skip terrain
    // rendering (it was drawn in a previous frame) and the recording misses it.
    if (polyrec_pending_name[0] && flagflip != AFF_ALL_NO_FLIP && flagflip != AFF_ALL_FLIP) {
        flagflip = AFF_ALL_NO_FLIP;
    }
#endif
    ...
    if (AnimateTexture OR ProtectActif)
        DoTextureAnimation();
    ...
    for (n = 0; n < MAX_EXTRAS; n++, ptrextra++) {
        ...
        if ((U16)ptrextra->Sprite == 0x8004u) // Dalle labyrinthe
        {
            TreeInsert((S16)(n | TYPE_LABY),
                       x, y, z,
                       x - 1024, y - 768, z - 1024,
    ...
    if (BaseSort()) {
        ...
        if (flagflip == AFF_ALL_FLIP OR flagflip == AFF_ALL_NO_FLIP) {
...
void SetZoneCamera(T_ZONE *ptrz) {
    ...
    if (CubeMode == CUBE_INTERIEUR) {
        if (StartXCube != info0 OR StartYCube != info1 OR StartZCube != info2) {
            StartXCube = info0;
            StartYCube = info1;
            StartZCube = info2;
        ...
        if (StartXCube != info0 OR StartYCube != info1 OR StartZCube != info2 OR SaveAlphaCam != AlphaCam OR SaveBetaCam != BetaCam OR SaveGammaCam != GammaCam OR SaveVueDistance != VueDistance) {
            StartXCube = info0;
            StartYCube = info1;
            StartZCube = info2;
        ...
        if (flagmess) {
            if (ptrz->Info1) // camera associee
            {
                ...
                for (i = 0; i < NbZones; i++, ptrzc++) {
                    if (ptrzc->Type == 1 AND ptrzc->Num == ptrz->Info1)
                        break;
                }
...
S32 GereZoneChangeCube(T_OBJET *ptrobj, T_ZONE *ptrz) {
    if (((ptrobj->Move != MOVE_BUGGY_MANUAL AND ptrobj->Move != MOVE_BUGGY)
        ...
        if (ptrobj->LifePoint > 0) {
            ...
            if (flagx)
                NewPosX = ptrz->Info0 + X0;
            if (flagz)
                NewPosZ = ptrz->Info2 + Z0;

            ptrobj->Obj.Beta += newbeta;
            ptrobj->Obj.Beta &= 4095;

            if (ptrz->Info6 & ZONE_DONT_REAJUST_POS_TWINSEN)
                FlagReajustPosTwinsen = FALSE;
            else
...
void CheckZoneSce(T_OBJET *ptrobj, U8 numobj) {
    ...
    if (y < 0)
        y = 0; // on ne teste pas les zones sous l'eau !
                    ...
                    if ((Input & I_UP
                        ...
                        if ((X0 >= 0) AND(Z0 >= 0)
                            ...
                            if (col == 0) {
                                ...
                                if (ptrobj->GenAnim == GEN_ANIM_DESCEND OR ptrobj->GenAnim == GEN_ANIM_ECHDESC) {
                                    InitAnim(GEN_ANIM_TOMBE, ANIM_TEMPO, NUM_PERSO);
                                    ptrobj->WorkFlags |= FALLING;
                                    if (!StartYFalling)
                                        StartYFalling = Nyw;
...
void CheckBuggyZoneGiver(T_OBJET *ptrobj) {
    ...
    for (n = 0; n < NbZones; n++, ptrz++) {
        ...
        if (ptrz->Type != 4 AND ptrz->Type != 5)
            continue;
