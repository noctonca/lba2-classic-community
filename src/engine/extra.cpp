...
#define	RND_BETA_BONUS	300	// Angle aléatoire rajouter

extern	S32	MagicBallFlags ;
S32	InitMagicBall = FALSE ;	// Pour ne pas traiter la balle
					// magique (ou plutot son ombre de
					// retour) dans la boucle ou elle a
					// été initialisée

// Tableaux de Kashes Maximum necessaire par chapitre scénarique
...
searchextra:
	...
	if( num==SPRITE_CLE
	AND !flagkill )// évite bcl infinie
	{
	...
	else
	{
		switch( ptrextra->Sprite&32767 )
		{
			case 3:	// c'est un objet 3D
			{
				...
				// recharger body ici pour etre sûr qu'il n'a pas bougé
	...
	switch( ptrextra->Sprite&32767 )
	{
		case 0: // Pof
			...
			if( scale!=ptrextra->Beta )
			{
				...
				{
					scale = BoundRegleTrois( scale, ptrextra->Beta, (U64)(ptrextra->PtrBody), TimerRefHR-ptrextra->Timer ) ;

					if( scale==ptrextra->Beta )
					{
						// on a atteind la taille désirée, ciao !!!!
						ptrextra->Sprite = -1 ;
					}
				}
...
S32	InitExtraPof( S32 x, S32 y, S32 z, S16 num, S32 alpha, S32 beta, S32 speed, S32 poids, S32 scaledeb, S32 scalefin, S32 duree, S32 rotation, S32 timeout )
{
	...
	for( n=0; n<MAX_EXTRAS; n++, ptrextra++ )
	{
		...
		ptrextra->PtrBody = (U8 *)(uintptr_t)(U32)duree ;
...
U8	WhichBonus( S32 flags )	// types de bonus demandés
{
	U8	n, m = 0 ;
	U8	tabbonus[5] ;
	S32	curr, max ;
...
//			if( n!=2 OR MagicLevel )
	...
	// si on n'a demandé que de la magie avec MagicLevel==0 alors on
	// donne de la vie
...
*/
	...
	// dernière solution, si tous les compteurs (Life, Gold & Magic)
...
/*══════════════════════════════════════════════════════════════════════════*/
// Retourne le numéro d'extra le plus près du point (x,z) (-1 si aucun extra)
...
void	GereExtras()
{
	...
	for( n=0; n<MAX_EXTRAS; n++, ptrextra++ )
	{
		...
		if( ptrextra->Flags & EXTRA_FLY )
		{
			...
			// Vérifie qu'on se déplace pas plus d'un pseudo, sinon
			// on passerait à travers
...
//					continue ;
						...
						AND ptrobj->LifePoint>0 )// pour ne pas prendre des coeurs !!!
						{
							...
							switch( ptrextra->Sprite )
							{
								...
								case SPRITE_MAGIE: /* Magic points */
									if( numobj==NUM_PERSO
									AND MagicLevel
									AND MagicPoint<MagicLevel*20 ) // à voir ?
									{
	...
	// a été initialisée dans cette boucle, elle sera traitée à la boucle
	// suivante
