...
U8 PtrWorldCodeBrick(S32 xw, S32 yw, S32 zw) {
    if (CubeMode == CUBE_INTERIEUR) {
        if (xw < 0 OR xw >= (SIZE_BRICK_XZ * SIZE_CUBE_X)
            ...
            return (WorldCodeBrick(xw, yw, zw));
    } else {
        if (APtObj->CarryBy != -1) {
            ...
            if (APtObj->CarryBy & CARRY_BY_DECORS)
                ...
                return 0; // porté par un objet scénarique
    ...
    if (CubeMode == CUBE_INTERIEUR) {
        if (GetScreenBrick(MouseX, MouseY)) {
            if (MyClick & 2) /* repositionne perso */
            {
                ...
                if (LastXmBrickSelected == X0 AND LastYmBrickSelected == Y0 AND LastZmBrickSelected == Z0) {
                    SaveTimer();
                    GereBrickMenu();
                    CameraCenter(1);
                    RestoreTimer();
        ...
        if (!GereMouseMovements(3)) {
            if (SelectPoly(MouseX, MouseY)) {
                ...
                if (MyClick & 2) {
                    ...
                    if (LastXmBrickSelected == xi AND LastYmBrickSelected == yi AND LastZmBrickSelected == zi) {
                        SaveTimer();
                        GereBrickMenu();
                        RestoreTimer();
                        FirstTime = AFF_ALL_FLIP;
