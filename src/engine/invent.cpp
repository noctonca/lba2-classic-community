...
S32 TricheInventory(S32 ret) {
    ...
    if (Key == K_F5) {
        TabInv[TabIndir[InvSelect]].IdObj3D = 0;
        ret = 1;
    }
    if (Key == K_F6) {
        TabInv[TabIndir[InvSelect]].IdObj3D = 1;
        ret = 1;
    }
    if (Key == K_F7) {
        TabInv[TabIndir[InvSelect]].IdObj3D = 2;
        ret = 1;
    }
    if (Key == K_F8) {
        TabInv[TabIndir[InvSelect]].IdObj3D = 3;
        ret = 1;
    }
...
U32 DrawOneInventory(S32 numbox) {
    ...
    x0 = INV_START_X + INV_INTER_X + (INV_SIZEO_X + INV_INTER_X) * (numbox / INV_NBOBJ_Y);
    y0 = INV_START_Y + INV_INTER_Y + (INV_SIZEO_Y + INV_INTER_Y) * (numbox % INV_NBOBJ_Y);
    ...
    {                    //	Compilo put Direct Value
        ...
        x0 = INV_START_X + INV_INTER_X + (INV_SIZEO_X + INV_INTER_X) * (15 / INV_NBOBJ_Y);
        y0 = INV_START_Y + INV_INTER_Y + (INV_SIZEO_Y + INV_INTER_Y) * (15 % INV_NBOBJ_Y);
...
void DrawInventoryScreen() {
    ...
    OldX0 = INV_START_X + INV_INTER_X + (INV_SIZEO_X + INV_INTER_X) * (0 / INV_NBOBJ_Y);
    OldY0 = INV_START_Y + INV_INTER_Y + (INV_SIZEO_Y + INV_INTER_Y) * (0 % INV_NBOBJ_Y);
...
void EmptyBlockInventory() {
    ...
    x0 = INV_START_X + INV_INTER_X + (INV_SIZEO_X + INV_INTER_X) * (InvSelect / INV_NBOBJ_Y);
    y0 = INV_START_Y + INV_INTER_Y + (INV_SIZEO_Y + INV_INTER_Y) * (InvSelect % INV_NBOBJ_Y);
...
#ifdef LBA_EDITOR
        if (!strcasecmp(ListObjet[NUM_PERSO].StringAnim, GEN_ANIM_MARCHE))
#else
...
//-------------
        ...
        if (myinput & I_RETURN) {
            ...
            if ((ListVarGame[TabIndir[InvSelect]] > 0)
				...
				{
					// peut etre remplacé par un test sur son body ?
					PingouinActif = TRUE ;
				}
*/
...
//	}
    ...
    ObjectInitAnim(&ptrobj->Obj, (void *)(uintptr_t)(U32)numanim);
    numanim = SearchAnim(GEN_ANIM_TROUVE, NUM_PERSO);

    if (numanim == -1)
        numanim = organi; // Anim Rien
    ...
    while (flag != 2) {
        ...
        if (FlagDisplayText)
            ...
            SetClipWindow(ClipWindowXMin, ClipWindowYMin, ClipWindowXMax, ClipWindowYMax);
...
*/
        ...
        if (!DemoSlide AND(MyKey == K_ESC OR Input & I_MENUS) AND !flag) {
            ...
            flag = 1;
            savetimer = TimerRefHR;
            ObjectInitAnim(&ptrobj->Obj, (void *)(uintptr_t)(U32)organi);
        }

        if (DemoSlide) {
            if (TimerRefHR > endtimer AND !TestSpeak())
                keypress = TRUE;
        }
...
void DrawArdoiseArrows() {
    ...
    //	else	RestoreBlock( Log, BufSpeak, L_X0, L_Y0, L_X1, L_Y1 ) ;
        ...
        CopyBlock(L_X0, L_Y0, L_X1, L_Y1, BufSpeak, L_X0, L_Y0, Log);
    ...
    //	else	RestoreBlock( Log, BufSpeak+5000, R_X0, R_Y0, R_X1, R_Y1 ) ;
        ...
        CopyBlock(R_X0, R_Y0, R_X1, R_Y1, BufSpeak, R_X0, R_Y0, Log);
...
void RouleauRight() {
    ...
    for (x = 0; x < TI_SHADE; x++)
        shade[x] = 12 - (x * 12 / (TI_SHADE - 1));
    for (x = 0; x < MI_SHADE; x++)
        shade[TI_SHADE + x] = 0;
    for (x = 0; x < TI_SHADE; x++)
        shade[ROULEAU_WIDTH - TI_SHADE + x] = x * 12 / (TI_SHADE - 1);
...
void GereArdoise() {
    ...
    while (!(Input & (I_RETURN | I_ACTION | I_MENUS)) AND Key != K_ESC) {
        ...
        if (Input & I_LEFT AND CurrentArdoise > 0) {
            ...
            while (TimerRefHR < savetimer + 10 * 20)
                ManageTime();
