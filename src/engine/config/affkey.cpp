...
void MenuConfig(void) {
    ...
    do {
        GraphPrintf(FALSE, 10, 30, "Press a key (<ESC> to quit) ...");

        MyGetInput();

        if (MyKey) {
            if (MyKey < 127)
                GraphPrintf(FALSE, 10, 10, "%2d (0x%02x) : %s           ", MyKey, MyKey, KeysTxt[MyKey]);
            else
                GraphPrintf(FALSE, 10, 20, "%2d (0x%02x)", MyKey, MyKey);
        }
