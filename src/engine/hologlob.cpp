...
static void AdjustZBufMinMax(void) {
    if (ScreenYMin < ZBufYMin)
        ZBufYMin = ScreenYMin;
    if (ScreenYMax > ZBufYMax)
        ZBufYMax = ScreenYMax;
    if (ZBufYMin < 0)
        ZBufYMin = 0;
    if (ZBufYMax > 479)
...
void AffUpperLeft(S32 x, S32 y, S32 flag) {
    ...
    if (flag)
        BoxMovingAdd(x, y, x + SX, y + SY); //	U L
}
//────────────────────────────────────────────────────────────────────────────
void AffLowerLeft(S32 x, S32 y, S32 flag) {
    ...
    if (flag)
        BoxMovingAdd(x, y - SY, x + SX, y); //	L L
}
//────────────────────────────────────────────────────────────────────────────
void AffUpperRight(S32 x, S32 y, S32 flag) {
    ...
    if (flag)
        BoxMovingAdd(x - SX, y, x, y + SY); //	U R
}
//────────────────────────────────────────────────────────────────────────────
void AffLowerRight(S32 x, S32 y, S32 flag) {
    ...
    if (flag)
        BoxMovingAdd(x - SX, y - SY, x, y); //	L R
}
//────────────────────────────────────────────────────────────────────────────
...
void AffReticule() {
    ...
    else if ((!FlagMap)AND(((TimerRefHR) & 0xFF) < 5 * 20))
        return;
...
void DrawArrowGlobe(T_SORT *ptt) {
    ...
    if (ptt->Num == 1)
        buf = (T_BODY_HEADER *)BufFleche; //	Big Arrow
    else
        buf = (T_BODY_HEADER *)BufLoFleche; //	Small Arrow
...
void DrawListHoloGlobe(S32 pos) {
    ...
    for (arrow = 0; arrow < MAX_OBJECTIF; arrow++, pt++) {
        ...
        //-------------
        if (pos)
            ComputeObjectifPlanet(arrow);
        //------------- Insertion
        if (TwinsenLinkArrow == arrow)
            ptt->Num = 2; // Little Arrow + Twinsen
        else
            ptt->Num = 1; // Big Arrow
...
U8 *HoloMalloc(U32 size) {
    ...
    SizeHoloMalloc -= size;
    if (SizeHoloMalloc < 0)
        TheEnd(NOT_ENOUGH_MEM, "HoloMalloc");
#endif
...
//────────────────────────────────────────────────────────────────────────────
U8 *LoadHoloMalloc_HQR(const char *name, S32 index) {
    U8 *pt;
    U32 size;
...
void InitHoloGlobe(S32 planet) {
    ...
    BufFleche = (U8 *)LoadHoloMalloc_HQR(tmpFilePath, HQR_FLECHE);
    ScaleRotBody3D(BufFleche, SCALE_OBJ_GLOBE, TRUE);

    BufLoFleche = (U8 *)LoadHoloMalloc_HQR(tmpFilePath, HQR_LOFLECHE);
    ...
    PtrMapping = (U16 *)LoadHoloMalloc_HQR(tmpFilePath, HQR_COORMAPP_HMM); // CoorMapp.HMM

    id = HQR_TWINSUN_HMT + (Planet * 2);

    PtrAlt = (U8 *)LoadHoloMalloc_HQR(tmpFilePath, id); // .HMT

    PtrTextMap = (U8 *)LoadHoloMalloc_HQR(tmpFilePath, id + 1); // .HMG
...
void InitHoloMap() {
    ...
    GetResPath(tmpFilePath, ADELINE_MAX_PATH, HOLO_HQR_NAME);
    if (!Load_HQR(tmpFilePath, TabArrow, HQR_ARROWBIN))
...
void HoloMap() {
    ...
    InitObjects(NULL, 0, HoloGivePtrBody, HoloGivePtrAnim);
    ...
    InitObjects(NULL, 0, GivePtrBody, GivePtrAnim);
