    ...
    for (x = 0; x < nbscanlines; x++, ptrtex++) {
        if (ptrtex[0] == DEF_EFFECT) {
            ...
            case 0: // plasma
            {
                ...
                // init structures
                PtrPlasma = (T_PLASMA *)Malloc(sizeof(T_PLASMA) + (nbactivepoints * nbactivepoints * delta * 2) + (nbactivepoints * nbactivepoints * 2) + (nbactivepoints * nbactivepoints * 4) + nbcols);
                ...
                // color table (ptrc points into Malloc tail after T_PLASMA)
                ptrcol = &ptrtex[1 + 3 * 256];
            ...
            case 1: // fire
            {
                ...
                // init structures
                PtrFire = (U8 *)Malloc(+sizeof(U8 *) // tex ptr
                                       + 32 * 36 + 32 * 36 + 32);
                ...
                *((U8 **)PtrFire) = ptrtex;
                ...
                // color table
                for (n = 0; n < 16; n++) {
                    PtrFire[sizeof(U8 *) + 32 * 36 * 2 + n] = 0;
                }
                for (n = 0; n < 16; n++) {
                    PtrFire[sizeof(U8 *) + 32 * 36 * 2 + n + 16] = *ptrcol++;
                }
            } break;
                //------------------------------------------------------------------
            }
...
void DoTextureAnimation() {
    ...
    for (n = 0; n < NbTexEffects; n++) {
        ...
        case 0: // plasma
        {
            ...
            Do_Plasma((T_PLASMA *)TexEffectList[n].PtrStruct);
        } break;
            //--------------------------------------------------------------------
        case 1: // fire
        {
            ...
            Do_Fire(ptrb + sizeof(U8 *), ptrb + 32 * 36 + sizeof(U8 *), ptrb + 32 * 36 * 2 + sizeof(U8 *), *(U8 **)ptrb);
        } break;
            //--------------------------------------------------------------------
        }
    }
