...
void DoFuncLife(T_OBJET *ptrobj) {
    ...
    case LF_USE_INVENTORY:
        ...
        if (InventoryAction == num) { // action émise depuis l'inventaire à cette boucle
            Value = TRUE;
        } else {
...
S32 DoTest() {
    ...
    case RET_STRING:
        ...
        valueword = strcasecmp(String, (char *)PtrPrg);
...
void DoLife(U8 numobj) {
    ...
    while (flag != -1) {
        ...
        case LM_SET_DIR: {
            ...
            if (numobj == NUM_PERSO AND ProtectActif AND ptrobj->Move != MOVE_MANUAL) {
        ...
        case LM_SET_DIR_OBJ: {
            ...
            if (ptrobjt->LifePoint > 0) {
                ...
                if (obj == NUM_PERSO AND ProtectActif AND ptrobjt->Move != MOVE_MANUAL) {
                    ToggleSortProtection();
                }
            } else
                PtrPrg++;
        ...
        case LM_SET_VAR_GAME:
            ...
            if (num == FLAG_MONEY) {
                if (Planet >= 2)
                    NbGoldPieces = ListVarGame[FLAG_MONEY];
                else
                    NbZlitosPieces = ListVarGame[FLAG_MONEY];
            }
...
*/
        ...
        case LM_POPCORN:
            /*				SaveTimer( ) ;
				...
				if( AdelineSystem( "pop\\popcorn.exe" )==-1 )
				{
