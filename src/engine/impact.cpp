...
S8 GetImpCommand(void) {
    ...
    if (c == ';')
        return IMP_REM; // accelere et evite l'espace obligatoire
...
S32 GetNumDefAddIfNot(char *pathdef, char *name, char *rem) {
    ...
    if (ptr)
        ...
        ptr = EndPtrDef; // on cherche le dernier numero

    // cherche numero

    while (ptr >= OrgPtrDef AND * ptr != ':')
        ptr--;
    ...
    if (ptr < OrgPtrDef)
        ...
        ptr--;
        while (ptr >= OrgPtrDef AND(*ptr >= '0' AND * ptr <= '9'))
            ptr--;
        if (ptr < OrgPtrDef)
            n = 0;
        else {
