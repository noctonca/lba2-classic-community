...
void AffString(S32 x, S32 y, char *str) {
    LIB386_TRACE_CPP("AffString", "1", "x=%d y=%d str=%p", x, y, (void *)str);
    U8 character;
    U8 line;
    S32 mask;
    PTR_U8 ptr;
    PTR_U8 font;
    PTR_U8 ScreenPtr = (PTR_U8)Log + TabOffLine[y] + x + SizeChar;
    ...
    while ((character = *str++) != 0) {
        ptr = ScreenPtr;
        font = &Font8x8[character * SizeChar];
        ptr = ScreenPtr - SizeChar;

        for (U32 pos = SizeChar; pos > 0; --pos) {
            line = *font++;
            mask = 0x80;
            while (mask > 0) {
                if (line & mask) {
                    *ptr = TextInk;
                } else if (TextPaper != 0xFF) {
                    *ptr = TextPaper;
                }
                ptr++;
                mask >>= 1;
...
void AffStringToBuffer(U8 *dst, U32 pitch, S32 x, S32 y, const char *str, U8 ink) {
    ...
    PTR_U8 ptr;
    PTR_U8 font;
    PTR_U8 rowStart = (PTR_U8)dst + (y * (S32)pitch) + x;
