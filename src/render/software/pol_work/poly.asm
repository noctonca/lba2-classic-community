...
fmt_draw_tri_1	DB	"[ASM][Draw_Triangle][1] entry", 10, 0

fmt_dt_1b	DB	"[ASM][Draw_Triangle][1b] p0=(%d,%d) u=%u v=%u w=%d light=%u zo=%u", 10, 0

fmt_dt_1c	DB	"[ASM][Draw_Triangle][1c] p1=(%d,%d) u=%u v=%u w=%d light=%u zo=%u", 10, 0

fmt_dt_1d	DB	"[ASM][Draw_Triangle][1d] p2=(%d,%d) u=%u v=%u w=%d light=%u zo=%u", 10, 0

fmt_dt_3	DB	"[ASM][Draw_Triangle][3] PtA=(%d,%d) PtB=(%d,%d) PtC=(%d,%d) type=%u", 10, 0

fmt_dt_9	DB	"[ASM][Draw_Triangle][9] return=%d LeftSlope=%d", 10, 0

fmt_ts_1	DB	"[ASM][Test_Scan][1] diffY=%d readFlag=%u curY=%d xmin=0x%08x xmax=0x%08x", 10, 0

fmt_ts_2	DB	"[ASM][Test_Scan][2] LeftSlope=%d RightSlope=%d", 10, 0

fmt_ts_4	DB	"[ASM][Test_Scan][4] calling filler nbLines=%d xmin=0x%08x xmax=0x%08x", 10, 0

fmt_trne_1	DB	"[ASM][Triangle_ReadNextEdge][1] readFlag=%u curY=%d", 10, 0

fmt_trne_2	DB	"[ASM][Triangle_ReadNextEdge][2] left new=(%d,%d) CurXMin=0x%08x", 10, 0

fmt_rnr_1	DB	"[ASM][Read_Next_Right][1] readFlag=%u curY=%d", 10, 0

fmt_rnr_2	DB	"[ASM][Read_Next_Right][2] right new=(%d,%d) CurXMax=0x%08x RSlope=%d", 10, 0

fmt_fpc_1	DB	"[ASM][Fill_PolyClip][1] nb=%d pts=%p type=%u clipFlag=%u color=%u", 10, 0

fmt_txzxs_9	DB	"[ASM][Calc_TextureZXSlopeFPU][9] U_XS=%d V_XS=%d W_XS=%d", 10, 0

fmt_txzgxs_9	DB	"[ASM][Calc_TextureZGouraudXSlopeFPU][9] U_XS=%d V_XS=%d G_XS=%d W_XS=%d", 10, 0

fmt_zbxs_9	DB	"[ASM][Calc_ZBufXSlopeFPU][9] ZBuf_XS=%d", 10, 0

fmt_txzxsz_9	DB	"[ASM][Calc_TextureZXSlopeZBufFPU][9] U_XS=%d V_XS=%d W_XS=%d ZBuf_XS=%d", 10, 0

fmt_txzls_9	DB	"[ASM][Calc_TextureZLeftSlopeFPU][9] LS=%d U_LS=%d V_LS=%d W_LS=%d curU=%d curV=%d curW=%d", 10, 0

fmt_txzgls_9	DB	"[ASM][Calc_TextureZGouraudLeftSlopeFPU][9] LS=%d G_LS=%d U_LS=%d V_LS=%d W_LS=%d", 10, 0

fmt_zbls_9	DB	"[ASM][Calc_ZBufLeftSlopeFPU][9] ZBuf_LS=%d CurZBuf=%u", 10, 0

fmt_gxs_9	DB	"[ASM][Calc_GouraudXSlopeFPU][9] G_XS=%d", 10, 0

fmt_txs_9	DB	"[ASM][Calc_TextureXSlopeFPU][9] U_XS=%d V_XS=%d", 10, 0

fmt_gls_9	DB	"[ASM][Calc_GouraudLeftSlopeFPU][9] LS=%d G_LS=%d curG=%d", 10, 0

fmt_tls_9	DB	"[ASM][Calc_TextureLeftSlopeFPU][9] LS=%d U_LS=%d V_LS=%d curU=%d curV=%d", 10, 0
...
;					*** PROCEDURES ***
;					******************
PUBLIC	C		Fill_PolyFast		; Polygon filler
PUBLIC	C		SetScreenPitch		; Change the pitch of the screen
PUBLIC	C		SetFog			; Initialize the fog
PUBLIC	C		SetCLUT			; Set the CLUT pointer
PUBLIC	C		Switch_Fillers_ASM		; To change the filling method


PUBLIC	C		Fill_PolyClip
PUBLIC	C		Fill_ClipXMin		; Clipping procedures
PUBLIC	C		Fill_ClipXMax
PUBLIC	C		Fill_ClipYMin
PUBLIC	C		Fill_ClipYMax

PUBLIC	C		Triangle_ReadNextEdge	; Entry point that reads the next point in the triangle
...
;					*** PUBLIC DATA ***
PUBLIC	C		ScreenPitch		; Screen width in bytes
PUBLIC	C		PTR_TabOffLine		; Ptr on a LUT containing a list of line-offsets
PUBLIC	C		PtrZBuffer		; ZBuffer pointer
PUBLIC	C		PtrCLUTGouraud		; Pointer to a Gouraud CLUT
PUBLIC	C		PtrCLUTFog		; Pointer to a Fog CLUT
PUBLIC	C		PtrTruePal		; Pointer to the palette CLUT
PUBLIC	C		RepMask			; Repeat zone for texture mapping
PUBLIC	C		PtrMap			; Pointer of the texture map
PUBLIC	C		Fill_Z_Fog_Near		; Near Fog
PUBLIC	C		Fill_Z_Fog_Far		; Far Fog
PUBLIC	C		Fill_ZBuffer_Factor	; ZBuffer factor
PUBLIC	C		Fill_Fog_Factor		; Fog factor
PUBLIC	C		Fill_ScaledFogNear	; Scaled FogNear value
PUBLIC	C		IsPolygonHidden		; Flag for Poly type 25


;					*** MODULE DATA ***
PUBLIC	C		Fill_Restart		; Return entry point from clipping proc
PUBLIC	C		Fill_Filler		; Chooser filler
PUBLIC	C		Fill_Patch		; Flag 1 means we should patch the filler proc; 0 means nothing to do
PUBLIC	C		Fill_Color		; The polygon color
PUBLIC	C		Fill_Trame_Parity	; Trame parity for Filler_Trame

PUBLIC	C		Fill_Type		; Polygon type number (needed for test globals sharing)

PUBLIC	C		Fill_FirstPoint		; Pointer of the first point of the list
PUBLIC	C		Fill_LastPoint		; Pointer of the last point of the list
PUBLIC	C		Fill_LeftPoint		; Pointer of the current point of the left edge
PUBLIC	C		Fill_RightPoint		; Pointer of the current point of the right edge

PUBLIC	C		Fill_LeftSlope		; Left slope
PUBLIC	C		Fill_RightSlope		; Right slope

PUBLIC	C		Fill_CurY		; Current Y
PUBLIC	C		Fill_CurXMin		; Current XMin
PUBLIC	C		Fill_CurXMax		; Current XMax
PUBLIC	C		Fill_CurOffLine		; Offset of the current video line

PUBLIC	C		Fill_CurGouraudMin	; Left edge: gouraud value
PUBLIC	C		Fill_Gouraud_LeftSlope	; Left slope for Gouraud shading
PUBLIC	C		Fill_Gouraud_XSlope	; X Slope for Gouraud shading

PUBLIC	C		Fill_CurMapUMin		; Left edge: MapU value
PUBLIC	C		Fill_MapU_LeftSlope	; Left slope for MapU
PUBLIC	C		Fill_MapU_XSlope	; X Slope for MapU

PUBLIC	C		Fill_CurMapVMin		; Left edge: MapV value
PUBLIC	C		Fill_MapV_LeftSlope	; Left slope for MapV
PUBLIC	C		Fill_MapV_XSlope	; X Slope for MapV

PUBLIC	C		Fill_CurWMin		; Left edge: W value
PUBLIC	C		Fill_W_LeftSlope	; Left slope for W
PUBLIC	C		Fill_W_XSlope		; X Slope for W

PUBLIC	C		Fill_CurZBufMin		; Left edge: ZBuf value
PUBLIC	C		Fill_ZBuf_LeftSlope	; Left slope for ZBuf
PUBLIC	C		Fill_ZBuf_XSlope	; X Slope for ZBuf
PUBLIC	C		Fill_CurZBuf		; Current value of ZBuf in the scanline

PUBLIC	C		Fill_Cur_W
PUBLIC	C		Fill_Next_W
PUBLIC	C		Fill_Cur_MapU
PUBLIC	C		Fill_Next_MapU
PUBLIC	C		Fill_Cur_MapV
PUBLIC	C		Fill_Next_MapV
PUBLIC	C		Fill_Cur_MapUOverW
PUBLIC	C		Fill_Next_MapUOverW
PUBLIC	C		Fill_Cur_MapVOverW
PUBLIC	C		Fill_Next_MapVOverW


PUBLIC	C		Fill_ReadFlag		; Flag telling which point to read
PUBLIC	C		Fill_ClipFlag		; Flag telling us what to clip (ie: edges, gouraud...)
PUBLIC	C		Fill_Flag_Fog		; True if FOG is active
PUBLIC	C		Fill_Flag_ZBuffer	; True if ZBUFFER is active
PUBLIC	C		Fill_Flag_NZW		; True if NZW is active

PUBLIC	C		Fill_Logical_Palette	; Logical palette

PUBLIC	C		F_1			; 1 in float
PUBLIC	C		F_256			; 256 in float

PUBLIC	C		Status_Int		; FPU control word for CHOP rounding mode
PUBLIC	C		Status_Float		; FPU control word for nearest rounding mode
...
M_FILL_CLIP		MACRO	Type
	...
	ENDIF
			...
			shl	ebp,16
			mov	eax,dword ptr [ebx].STRUC_POINT.Pt_XE
...
@@Jump_First:		mov	eax,[Fill_LastPoint]
			...
			mov	eax,dword ptr [esi+Size STRUC_POINT].STRUC_POINT.Pt_XE
...
@@Visible:
			...
			inc	ecx
@@Pas_Inters:
			mov	eax,dword ptr [esi].STRUC_POINT.Pt_XE	; Visible
			mov	edx,dword ptr [esi].STRUC_POINT.Pt_Light

			mov	dword ptr [edi].STRUC_POINT.Pt_XE,eax
			mov	dword ptr [edi].STRUC_POINT.Pt_Light,edx

			mov	eax,dword ptr [esi].STRUC_POINT.Pt_MapU
			mov	edx,[esi].STRUC_POINT.Pt_W

			mov	dword ptr [edi].STRUC_POINT.Pt_MapU,eax
			mov	[edi].STRUC_POINT.Pt_W,edx
...
Extrn	C		Fill_N_Table_Jumps	:	DWORD
Extrn	C		Fill_Fog_Table_Jumps	:	DWORD
Extrn	C		Fill_ZBuf_Table_Jumps	:	DWORD
Extrn	C		Fill_FogZBuf_Table_Jumps:	DWORD
Extrn	C		Fill_NZW_Table_Jumps	:	DWORD
Extrn	C		Fill_FogNZW_Table_Jumps	:	DWORD
...
Fill_Saut_Normal        DWORD Offset Fill_N_Table_Jumps
;Patch_Jmp_XSlopeFPU     DWORD Offset Jmp_XSlopeFPU
;Patch_Jmp_LeftSlopeFPU  DWORD Offset Jmp_LeftSlopeFPU
...
;			mov	[Patch_Jmp_XSlopeFPU],ecx
;			mov	[Patch_Jmp_LeftSlopeFPU],edx
			jmp	@@TrueEnd
@@TrueEnd:
			pop	edx
			pop	ecx
...
; ╚══════════════════════════════════════════════════════════════════════════╝
			ALIGN	4
Fill_PolyClip		PROC
		; --- debug trace [ASM][Fill_PolyClip][1] params ---
		pushad
		push	dword ptr [Fill_Color]
		push	dword ptr [Fill_ClipFlag]
		push	dword ptr [Fill_Type]
		push	esi
		push	ecx
		push	offset fmt_fpc_1
		call	printf
		add	esp, 24
		popad
		; --- end debug trace ---
...
Draw_Triangle		PROC esi
		...
		; --- debug trace [ASM][Draw_Triangle][1] params ---
		pushad
		push	offset fmt_draw_tri_1
		call	printf
		add	esp, 4
		popad
		; --- end debug trace ---
		; --- debug trace [ASM][Draw_Triangle][1b] vertex 0 ---
		pushad
		movsx	eax, word ptr [esi+0+22]  ; ZO
		push	eax
		movsx	eax, word ptr [esi+0+20]  ; Light
		push	eax
		push	dword ptr [esi+0+16]       ; W
		movzx	eax, word ptr [esi+0+14]  ; MapV
		push	eax
		movzx	eax, word ptr [esi+0+12]  ; MapU
		push	eax
		movsx	eax, word ptr [esi+0+2]   ; YE
		push	eax
		movsx	eax, word ptr [esi+0]     ; XE
		push	eax
		push	offset fmt_dt_1b
		call	printf
		add	esp, 32
		popad
		; --- end debug trace ---

		; --- debug trace [ASM][Draw_Triangle][1c] vertex 1 ---
		pushad
		movsx	eax, word ptr [esi+32+22]  ; ZO
		push	eax
		movsx	eax, word ptr [esi+32+20]  ; Light
		push	eax
		push	dword ptr [esi+32+16]       ; W
		movzx	eax, word ptr [esi+32+14]  ; MapV
		push	eax
		movzx	eax, word ptr [esi+32+12]  ; MapU
		push	eax
		movsx	eax, word ptr [esi+32+2]   ; YE
		push	eax
		movsx	eax, word ptr [esi+32]     ; XE
		push	eax
		push	offset fmt_dt_1c
		call	printf
		add	esp, 32
		popad
		; --- end debug trace ---

		; --- debug trace [ASM][Draw_Triangle][1d] vertex 2 ---
		pushad
		movsx	eax, word ptr [esi+64+22]  ; ZO
		push	eax
		movsx	eax, word ptr [esi+64+20]  ; Light
		push	eax
		push	dword ptr [esi+64+16]       ; W
		movzx	eax, word ptr [esi+64+14]  ; MapV
		push	eax
		movzx	eax, word ptr [esi+64+12]  ; MapU
		push	eax
		movsx	eax, word ptr [esi+64+2]   ; YE
		push	eax
		movsx	eax, word ptr [esi+64]     ; XE
		push	eax
		push	offset fmt_dt_1d
		call	printf
		add	esp, 32
		popad
		; --- end debug trace ---
...
;	push ebx
;	mov ebx,dword ptr [Patch_Jmp_XSlopeFPU]
;	lea edx,[ebx+edx*4]
;	pop ebx
;	jmp	[edx]
					...
					; *** Integer math. ***
@@DenomInt: ; TODO: Remove, only used for int math
			mov	eax,dword ptr [edi].STRUC_POINT.Pt_XE
			mov	edx,dword ptr [esi].STRUC_POINT.Pt_XE
			...
			mov	ebx,dword ptr [ebp].STRUC_POINT.Pt_XE
			...
			push	eax
			mov	eax,dword ptr [ebp].STRUC_POINT.Pt_XE
...
Patch_Jmp_LeftSlopeFPU::jmp	[Jmp_LeftSlopeZBufFPU+edx*4]

;			push eax
;			mov eax,dword ptr [Patch_Jmp_LeftSlopeFPU]
;			lea edx,[eax+edx*4]
;			pop eax
;			jmp	[edx]
...
@@Test_Scan:
		...
		; --- debug trace [ASM][Test_Scan][1] entry ---
		pushad
		push	dword ptr [Fill_CurXMax]
		push	dword ptr [Fill_CurXMin]
		push	dword ptr [Fill_CurY]
		push	dword ptr [Fill_ReadFlag]
		push	ecx
		push	offset fmt_ts_1
		call	printf
		add	esp, 24
		popad
		; --- end debug trace ---
		; --- debug trace [ASM][Test_Scan][2] slopes ---
		pushad
		push	dword ptr [Fill_RightSlope]
		push	dword ptr [Fill_LeftSlope]
		push	offset fmt_ts_2
		call	printf
		add	esp, 12
		popad
		; --- end debug trace ---
...
@@EndRet:
			pop	ebp
			; --- debug trace [ASM][Draw_Triangle][9] return ---
		pushad
		push	dword ptr [Fill_LeftSlope]
		push	eax
		push	offset fmt_dt_9
		call	printf
		add	esp, 12
		popad
		; --- end debug trace ---
...
Calc_GouraudXSlope	PROC
			mov	eax,dword ptr [edi].STRUC_POINT.Pt_Light
			mov	ecx,dword ptr [esi].STRUC_POINT.Pt_Light
			...
			mov	eax,dword ptr [ebp].STRUC_POINT.Pt_Light
			mov	ecx,dword ptr [esi].STRUC_POINT.Pt_Light
...
Calc_GouraudXSlopeFPU	PROC
			...
			sub	eax,ecx			; EAX = LB-LA
			mov	ebx,dword ptr [ebp].STRUC_POINT.Pt_Light
...
Gouraud_XSlopeFPU::
		...
		; --- debug trace [ASM][Calc_GouraudXSlopeFPU][9] return ---
		pushad
		push	dword ptr [Fill_Gouraud_XSlope]
		push	offset fmt_gxs_9
		call	printf
		add	esp, 8
		popad
		; --- end debug trace ---
...
@@SatESIUp:
			mov	ecx,14*256-1
			mov	[esi].STRUC_POINT.Pt_Light,cx
			jmp	@@RetESI
@@SatESILow:
			mov	ecx,256
			mov	[esi].STRUC_POINT.Pt_Light,cx
...
@@SatEBPUp:
			mov	eax,14*256-1
			mov	[ebp].STRUC_POINT.Pt_Light,ax
			jmp	@@RetEBP
@@SatEBPLow:
			mov	eax,256
			mov	[ebp].STRUC_POINT.Pt_Light,ax
...
@@SatEBPUp:
			mov	ebx,14*256-1
			mov	[ebp].STRUC_POINT.Pt_Light,bx
			jmp	@@RetEBP
@@SatEBPLow:
			mov	ebx,256
			mov	[ebp].STRUC_POINT.Pt_Light,bx
...
Calc_TextureXSlope	PROC
					; ** MapU **
			mov	eax,dword ptr [edi].STRUC_POINT.Pt_MapU
			mov	ecx,dword ptr [esi].STRUC_POINT.Pt_MapU
			...
			mov	eax,dword ptr [ebp].STRUC_POINT.Pt_MapU
			mov	ecx,dword ptr [esi].STRUC_POINT.Pt_MapU
...
Calc_TextureXSlopeFPU	PROC
			...
			sub	eax,ecx			; EAX = LB-LA
			mov	ebx,dword ptr [ebp].STRUC_POINT.Pt_MapU
...
;		fadd	[FInv_2]
		...
		; --- debug trace [ASM][Calc_TextureXSlopeFPU][9] return ---
		pushad
		push	dword ptr [Fill_MapV_XSlope]
		push	dword ptr [Fill_MapU_XSlope]
		push	offset fmt_txs_9
		call	printf
		add	esp, 12
		popad
		; --- end debug trace ---
...
@@Ok:
			...
			mov	[Fill_MapV_XSlope],eax
					...
					; ** W **
			mov	eax,[edi].STRUC_POINT.Pt_W
			mov	ecx,[esi].STRUC_POINT.Pt_W
...
Calc_TextureZXSlopeFPU	PROC
			...
			; (Yb-Ya)(Xc-Xa)-(Yc-Ya)(Xb-Xa)
					...
					; ** MapU **
			mov	eax,dword ptr [esi].STRUC_POINT.Pt_MapU
			mov	ebx,dword ptr [edi].STRUC_POINT.Pt_MapU

			shr	eax,16
			mov	ecx,dword ptr [ebp].STRUC_POINT.Pt_MapU
			...
			fild	[esi].STRUC_POINT.Pt_W	; WA  UA  (Yb-Ya)  (Yc-Ya)  D

			fild	dword ptr [esp+1*4]; UB  WA  UA  (Yb-Ya)  (Yc-Ya)  D

			fild	[edi].STRUC_POINT.Pt_W	; WB  UB  WA  UA  (Yb-Ya)  (Yc-Ya)  D
			...
			fild	[ebp].STRUC_POINT.Pt_W	; WC  UC  UB  WB  UA'  (Yb-Ya)  (Yc-Ya)  D
			...
			fild	[esi].STRUC_POINT.Pt_W	; WA  VA  (MUC-MUB)  (Yb-Ya)  (Yc-Ya)  D'

			fild	dword ptr [esp+4*4]; VB  WA  VA  (MUC-MUB)  (Yb-Ya)  (Yc-Ya)  D'

			fild	[edi].STRUC_POINT.Pt_W	; WB  VB  WA  VA  (MUC-MUB)  (Yb-Ya)  (Yc-Ya)  D'
			...
			fild	[ebp].STRUC_POINT.Pt_W	; WC  VC  USlope  VA' VB' (Yb-Ya)  (Yc-Ya)  D'
			...
			fild	[esi].STRUC_POINT.Pt_W	; WA  MVB  MVC (Yb-Ya)  (Yc-Ya)  D'

			fild	[edi].STRUC_POINT.Pt_W	; WB  WA  MVB  MVC (Yb-Ya)  (Yc-Ya)  D'

			fild	[ebp].STRUC_POINT.Pt_W	; WC  WB  WA  MVB  MVC (Yb-Ya)  (Yc-Ya)  D'
...
Calc_TextureZGouraudXSlopeFPU PROC
			...
			mov	eax,dword ptr [esi].STRUC_POINT.Pt_Light
			mov	ebx,dword ptr [edi].STRUC_POINT.Pt_Light

			mov	ecx,dword ptr [ebp].STRUC_POINT.Pt_Light
			and	eax,0FFFFh
...
Calc_ZBufXSlopeFPU	PROC
		...
		; --- debug trace [ASM][Calc_ZBufXSlopeFPU][9] return ---
		pushad
		push	dword ptr [Fill_ZBuf_XSlope]
		push	offset fmt_zbxs_9
		call	printf
		add	esp, 8
		popad
		; --- end debug trace ---
...
Calc_TextureZXSlopeZBufFPU PROC
			...
			fild	[esi].STRUC_POINT.Pt_W	; WA  ZA  (Yb-Ya)  (Yc-Ya)  D

			fild	dword ptr [esp+7*4]; ZB  WA  ZA  (Yb-Ya)  (Yc-Ya)  D

			fild	[edi].STRUC_POINT.Pt_W	; WB  ZB  WA  ZA  (Yb-Ya)  (Yc-Ya)  D
			...
			fild	[ebp].STRUC_POINT.Pt_W	; WC  ZC  ZB  WB  ZA'  (Yb-Ya)  (Yc-Ya)  D
...
CalcGouraudTableFPU::
			...
			fistp	[Fill_LeftSlope]
					...
					; --- debug trace [ASM][Calc_GouraudLeftSlopeFPU][9] return ---
		pushad
		push	dword ptr [Fill_CurGouraudMin]
		push	dword ptr [Fill_Gouraud_LeftSlope]
		push	dword ptr [Fill_LeftSlope]
		push	offset fmt_gls_9
		call	printf
		add	esp, 16
		popad
		; --- end debug trace ---
...
ENDIF
			...
			shl	eax,8
			mov	edx,dword ptr [edi].STRUC_POINT.Pt_MapU
...
;		fadd	[FInv_2]
			fistp	[Fill_MapV_LeftSlope]
					...
					; --- debug trace [ASM][Calc_TextureLeftSlopeFPU][9] return ---
		pushad
		push	dword ptr [Fill_CurMapVMin]
		push	dword ptr [Fill_CurMapUMin]
		push	dword ptr [Fill_MapV_LeftSlope]
		push	dword ptr [Fill_MapU_LeftSlope]
		push	dword ptr [Fill_LeftSlope]
		push	offset fmt_tls_9
		call	printf
		add	esp, 24
		popad
		; --- end debug trace ---
...
Calc_TextureZLeftSlope	PROC
			...
			mov	eax,dword ptr [edi].STRUC_POINT.Pt_MapU
			mov	ebx,dword ptr [esi].STRUC_POINT.Pt_MapU
...
ENDIF
			...
			mov	[Fill_MapV_LeftSlope],eax
					...
					; ** W **
			mov	eax,[esi].STRUC_POINT.Pt_W
			mov	ebx,[edi].STRUC_POINT.Pt_W
...
ENDIF
			...
			fild	[edi].STRUC_POINT.Pt_W	; WA  UA  dX  1/dY
			...
			fild	[esi].STRUC_POINT.Pt_W	; WB UB XSlope' UA' WA  1/dY
...
ENDIF
			...
			fild	[edi].STRUC_POINT.Pt_W	; WA  LSlope  UA  dX  1/dY
...
Calc_ZBufLeftSlopeFPU	PROC
		...
		; --- debug trace [ASM][Calc_ZBufLeftSlopeFPU][9] return ---
		pushad
		push	dword ptr [Fill_CurZBufMin]
		push	dword ptr [Fill_ZBuf_LeftSlope]
		push	offset fmt_zbls_9
		call	printf
		add	esp, 12
		popad
		; --- end debug trace ---
...
ENDIF
			...
			mov	[Fill_MapV_LeftSlope],eax
					...
					; ** ZBuf **
			mov	ebx,dword ptr [esi].STRUC_POINT.Pt_Light
...
ENDIF
			...
			fild	[edi].STRUC_POINT.Pt_W	; WA  ZB  ZA  1/dY

			fild	[esi].STRUC_POINT.Pt_W	; WB  WA  ZB  ZA  1/dY
